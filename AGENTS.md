...
- Any repository content change must bump the `bosscli` version before commit. Use `npm version <version> --no-git-tag-version` so `package.json` and `package-lock.json` stay aligned.

...
DEFAULT_DOCX = Path("/Users/gepeng/Downloads/文档/乐享协同数字化电子发票接口规范v1.0.62（销项）.docx")
OUTPUT_PATH = ROOT / "src" / "lexiang-general-snapshot.ts"
WORD_NS = {"w": "http://schemas.openxmlformats.org/wordprocessingml/2006/main"}
SOURCE_DOC = "乐享协同数字化电子发票接口规范v1.0.62（销项）.docx"
...
def is_interface_list_table(rows: list[list[str]]) -> bool:
    if not rows:
        return False
    header = [normalize_header(cell) for cell in rows[0]]
    return "接口名称" in header and "接口描述" in header
...
def collect_interface_list(items: list[WordItem]) -> dict[str, dict[str, str]]:
    ...
    for item in items:
        ...
        if not is_interface_list_table(rows):
            continue
        header = rows[0]
        name_i = header_index(header, "接口名称")
        description_i = header_index(header, "接口描述", "描述")
        for row in rows[1:]:
            name = read_cell(row, name_i)
            if not name:
                continue
            result[name] = {
                "description": read_cell(row, description_i),
            }
    return result


def collect_toc_titles(items: list[WordItem], interface_names: set[str]) -> dict[str, str]:
    result: dict[str, str] = {}
    names_by_length = sorted(interface_names, key=len, reverse=True)
    for item in items:
        if item.kind != "p":
            continue
        text = str(item.value)
        match = re.match(r"^(3\.\d+\.\d+)\s+(.+)$", text)
        if not match:
            continue
        section_no, rest = match.groups()
        for name in names_by_length:
            if rest.startswith(name):
                result.setdefault(name, f"{section_no} {name}")
                break
    return result
...
def extract_path(text: str) -> str | None:
    if "接口服务路径" not in text:
        return None
    raw = text.split("接口服务路径", 1)[1].lstrip("：:").strip()
    match = re.search(r"(/api/[^\s]+)", raw)
    return match.group(1) if match else None


def is_generic_paragraph(text: str) -> bool:
    if not text:
        return True
    return (
        text in {"接口描述", "输入参数", "输出参数"}
        or text.startswith("接口描述：")
        or text.startswith("接口请求方式")
        or text.startswith("以下请求参数")
...
def is_category_heading(text: str, interface_names: set[str]) -> bool:
    if re.match(r"^3\.\d+\s+", text):
        return True
    return text not in interface_names and text in {
        "登录认证",
        "开票接口",
        "发票查询下载接口",
        "订单开票",
...
def clean_category(text: str) -> str:
    return re.sub(r"^3\.\d+\s+", "", text).strip()
...
def collect_sections(items: list[WordItem], interface_names: set[str]) -> list[Section]:
    ...
    for index, item in enumerate(items):
        ...
        if is_category_heading(text, interface_names):
            current_category = clean_category(text)
        category_by_index[index] = current_category

        path = extract_path(text)
        if not path:
            continue

        title_index = find_title_index(items, index, interface_names)
        if title_index is None:
            continue
        name = str(items[title_index].value).strip()
        candidates.append(
            {
                "name": name,
                "category": category_by_index.get(title_index, current_category),
                "path": path,
                "start": title_index,
            }
        )
    ...
    for position, candidate in enumerate(candidates):
        end = candidates[position + 1]["start"] if position + 1 < len(candidates) else len(items)
        sections.append(
            Section(
                name=candidate["name"],
...
def find_title_index(items: list[WordItem], path_index: int, interface_names: set[str]) -> int | None:
    fallback: int | None = None
    for index in range(path_index - 1, max(-1, path_index - 10), -1):
        item = items[index]
        if item.kind != "p":
            continue
        text = str(item.value).strip()
        if is_generic_paragraph(text):
            continue
        if text in interface_names:
            return index
        if fallback is None and not re.match(r"^3\.\d+\s+", text):
            fallback = index
    return fallback
...
def field_from_row(row: list[str], header: list[str], group: str) -> dict[str, str] | None:
    ...
    desc_i = header_index(header, "描述", "说明", "补充")
...
def extract_a_number(text: str) -> str | None:
    match = re.match(r"^A[-－](\d+(?:\.\d+)*)", text.strip(), re.I)
    return match.group(1) if match else None
...
def map_a_number_to_list_field(fields: list[dict[str, str]]) -> dict[str, str]:
    ...
    for field in fields:
        if "list" not in field.get("type", "").lower():
            continue
        for match in re.finditer(r"详见A[-－](\d+(?:\.\d+)*)", field.get("description", ""), re.I):
...
def build_snapshot(items: list[WordItem]) -> list[dict[str, Any]]:
    ...
    sections = collect_sections(items, set(interface_list))
    toc_titles = collect_toc_titles(items, set(interface_list) | {section.name for section in sections})
    snapshot: list[dict[str, Any]] = []

    for section in sections:
        tables = collect_input_tables(items, section.start, section.end)
        fields, template = build_section_template(tables)
        list_entry = interface_list.get(section.name, {})
        snapshot.append(
            {
                ...
                "sectionTitle": toc_titles.get(section.name, section.name),
                "fields": fields,
                "template": template,
            }
        )
...
def write_output(snapshot: list[dict[str, Any]]) -> None:
    ...
    output = (
        ...
        f"export const LEXIANG_GENERAL_INTERFACES = {payload} as const satisfies readonly LexiangInterfaceInfo[];\n"
    )
